...
BOARD_LENGTH = 12
BOARD_HEIGTH = 8
BOARD_SIZE = BOARD_HEIGTH * BOARD_LENGTH  # hauteur*largeur
TUPLE_ORIGINAL = (
...
def test():
    ...
    assert (tuple_to_array(TUPLE_ORIGINAL) == [[EMPTY_CELL for _ in range(BOARD_LENGTH)] for _ in range(BOARD_HEIGTH)])
...
def tuple_to_array(tuple_to_update, length=BOARD_LENGTH, height=BOARD_HEIGTH):
...
def get_index_token_positionable(board, length=BOARD_LENGTH, height=BOARD_HEIGTH):
    indexes = []
...
def minimax(board, is_red=True, length=BOARD_LENGTH, height=BOARD_HEIGTH):
    if board in tree:
        return
